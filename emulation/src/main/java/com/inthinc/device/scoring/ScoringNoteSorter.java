...
public class ScoringNoteSorter {
    ...
    public void preProcessNotes(List<Map<String, Object>> notes, ProductType type) {
        ...
        while (itr.hasNext()) {
            ...
            if (((Integer) next.get(forgiven)) == 1)
                continue;
            ...
            } else if (eventType.equals(DeviceNoteTypes.SEATBELT)) {
                map.put(topSpeed, (Integer) next.get(topSpeed));
                map.put(odometer, (Integer) next.get(odometer));

                seatbelt.put((Long) next.get(noteID), map);

            } else if (eventType.equals(DeviceNoteTypes.NOTE_EVENT)) {
            ...
            } else if (eventType.equals(DeviceNoteTypes.IDLING)) {
                map.put(lowIdle, (Integer) next.get(lowIdle));
                map.put(highIdle, (Integer) next.get(highIdle));
                idleing.put((Long) next.get(noteID), map);

            } else if (eventType.equals(DeviceNoteTypes.VERTICAL_EVENT) || eventType.equals(DeviceNoteTypes.VERTICAL_EVENT_SECONDARY)) {
                map.put(deltaX, 0);
                map.put(deltaY, 0);
                map.put(deltaZ, 0);
                map.put(speed, 0);
