...
public class Masthead {
    ...
    public void click_my_messages() {
        ...
        selenium.verifyLocation("tiwipro/app/messages/", "My Messages click");// TODO: jwimmer: DTanner: no hard coded Strings in the FRAMEWORK code
    }
    ...
    public void click_help(String help_page) {
        if (help_page.indexOf(".htm") == -1) {
            help_page += ".htm";
        }// TODO: jwimmer: DTanner: no hard coded Strings in the FRAMEWORK code... I'm not sure this line is doing anything (affective) anyway?
    ...
    public void test_self_before_login() {
        if (selenium.getLocation().indexOf("/tiwipro/login") == -1) {
            selenium.open("login", "Open login page");
        }
        ck_footer();
        click_privacy();
        click_legal();
