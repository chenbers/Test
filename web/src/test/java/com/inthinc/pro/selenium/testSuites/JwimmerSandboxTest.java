...
public class JwimmerSandboxTest extends WebTest {
    ...
    public void liveFleet_sandbox_taeThrownError() {
        ...
        liveFleet._select().numNearestVehicles().selectPartMatch("5");
        liveFleet._textField().findAddress().type("part(5);fifty?");
        pause(waitTime);
        liveFleet._select().numNearestVehicles().selectPartMatch("0", 3);
    ...
    public void liveFleet_changeDefaultViewAsNONAdmin_shouldNotSeeLink() {
        ...
        liveFleet.page_bareMinimum_validate();// only continue if the page meets bare minimum this step has more validity in LONGER tests... where we want to fail the test as early
                                              // as possible
