...
public class HTTPCommands {
	...
	public HTTPCommands(String username, String password){
    	...
    	httpClient.getState().setCredentials(new AuthScope("rally1.rallydev.com", 443), new UsernamePasswordCredentials(username, password));
    }
	 ...
	 */
	public void httpRequest(HttpMethod method) throws IOException, HttpException {
		response=null;
		try {
			int statusCode = httpClient.executeMethod(method);
			if (statusCode != HttpStatus.SC_OK) {
	            logger.debug("POST method failed: " + method.getStatusLine());
	            response = getResponseBodyFromStream(method.getResponseBodyAsStream());
	        } else {
	...
	public void constructQuery(Integer startPosition, Integer pageSize) throws URIException, JSONException{
    	constructQuery("", startPosition, pageSize);
    }
    public void constructQuery(Integer startPosition, Integer pageSize, Boolean fetch) {
    	constructQuery("", startPosition, pageSize, fetch);
    }
    public void constructQuery(String queryString, Integer startPosition, Integer pageSize) {
    ...
    public void constructQuery(String queryString, Integer startPosition, Integer pageSize, Boolean fetch) {
    	query = "workspace="+workingSpace.getValue();
    	query += "&query=" + queryString;
    	query += "&start=" + startPosition;
    	query += "&pageSize=" + pageSize;
    ...
    public String constructFilter(String filterString) throws URIException {
    	if (filterString == null || filterString.equals("")) return "";
    ...
    public String constructFilter( NameValuePair[] filterBy) throws URIException {
	    ...
	    for (NameValuePair pair: filterBy) {
			if (filterRaw == null ) {
				...
				filterRaw = "( " + filterRaw + " AND " + constructFilter(pair, false) + " )";
			}
		}
	...
	public String constructFilter(NameValuePair pair, Boolean encode) throws URIException {
		String formatted = "( " + pair.getName() + " = \"" + pair.getValue() + "\" )";
		if (encode) return encodeURLQuery(formatted);
    ...
    public void postObjects(RallyWebServices request, JSONObject item, Boolean create) {
    	String url = null;
    	try {
			if (create) url = request.getValue() + "/create.js";
			else {
			...
			RequestEntity requestEntity = new StringRequestEntity(postJSON.toString(), "application/json", "UTF-8");
    ...
    public void setWorkspace(RallyWebServices space, Boolean fetch) {
    	workingSpace = space;
    	try {
	    	if (fetch) constructQuery(1, 200, fetch);
	    	else constructQuery(1, 200);
    ...
    /**
     * Get the Query Result from Rally.  Objects have the form
