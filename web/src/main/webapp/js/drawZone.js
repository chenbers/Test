 ...
 *              array of GLatLng instances containing the polygon's current coordinates.
 ...
 *              of GLatLng instances containing the polygon's current coordinates.
 */
 ...
 *
 *  outline: the zone's polygon outline, either as an array of GLatLng instances or as a string in the form
 ...
 */
function loadZone(outline, editable)
{
...
/**
 * Returns a string from the given array of GLatLng instances in the form of "(x1,y1);(x2,y2);(x3,y3)" where
 * x1, y1, etc. are latitudes and longitudes on the map.
 *
 *  outline: an array of GLatLng instances comprising the zone's outline.
 */
...
/**
 * Returns an array of GLatLng instances from a string in the form created by toOutlineString.
 *
  ...
  if (outlineString)
  {
    ...
    if (segs.length >= 3)
    {
      ...
      for (var i = 0; i < segs.length; i++)
      {
        ...
        outline.push(new GLatLng(parseFloat(ll[0]), parseFloat(ll[1])));
      }
      return outline;
    }
  }
...
/**
 * Returns an array of GLatLng instances describing the outline of the currently displayed zone or null.
 */
...
/**
 * Returns a GLatLngBounds for the given outline.
 *
 *  outline: the zone's polygon outline, either as an array of GLatLng instances or as a string in the form
...
function getLatLngBounds(outline)
{
  ...
  if (outline)
  {
    var bounds = new GBounds(outline);
    return new GLatLngBounds(new GLatLng(bounds.minY, bounds.minX), new GLatLng(bounds.maxY, bounds.maxX));
  }
  return null;
}
...
function endPolygon_(poly, unfinished)
{
  ...
  else
  {
    if (unfinished)
    {
      ...
      poly.insertVertex(poly.getVertexCount(), new GLatLng(latlng.lat(), latlng.lng()));
    }
