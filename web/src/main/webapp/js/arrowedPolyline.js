...
function BDCCArrowedPolyline(points, color, weight, opacity, opts, gapPx, 
				headLength, headColor, headWeight, headOpacity) {    
...
BDCCArrowedPolyline.prototype.copy = function(map) {
    return new BDCCArrowedPolyline(this.points,this.color,this.weight,this.opacity,this.opts,this.gapPx, 
    		this.headLength, this.headColor, this.headWeight, this.headOpacity);
}
...
BDCCArrowedPolyline.prototype.recalc = function() {
   ...
   var segmentsSinceLastArrow = 0; 
   var segmentsLength = 0;
...
//                        var y = p1.y - ((sl/2) * Math.sin(theta));
//                        this.addHead(x,y,theta,zoom);
//                        segmentsSinceLastArrow = 0;
//              }
//              else{
//        	
//            	  segmentsSinceLastArrow++;
//        }
      }
...
BDCCArrowedPolyline.prototype.addHead = function(x,y,theta,zoom) {
    ...
    var t = theta + (Math.PI/6) ;
    if(t > Math.PI)
        t -= 2*Math.PI;
    var t2 = theta - (Math.PI/6) ;
    if(t2 <= (-Math.PI))
        t2 += 2*Math.PI;
    var pts = new Array();
    ...
    pts.push(this.prj.fromPixelToLatLng(new GPoint(x1,y1), zoom));
      this.heads.push(new GPolygon(pts, this.headColor,this.headWeight,this.headOpacity,  this.headColor, this.headOpacity, {clickable:false})); 
