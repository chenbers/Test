...
function initPopUp() {
	...
	popcont.innerHTML = '' +
		'<div id="popupInner">' +
			'<div id="popupTitleBar" style="height: 0px; padding: 0px; margin: 0px; border: 0px;">' +
				'<div id="popupTitle" style="display:none"></div>' +
				'<div id="popupControls" style="display:none">' +
					'<input name="" type="button" class="button_edit" /> &nbsp; <input name="" type="button" class="button_close" onclick="hidePopWin(false);" />' +
				'</div>' +
			'</div>' +
			...
			//'<div id="popupFooter">' +
				//'<table width="100%" border="0" cellspacing="0" cellpadding="10" style="border-top: 1px solid #333; margin: 20px 0 0 0;">' +
				  //'<tr>' +
	...
	if (window.frames["popupFrame"].document.title == null) {
		...
		if (popupDocument.getElementsByTagName('base').length < 1) {
			...
			for (var i = 0; i < aList.length; i++) {
				if (aList.target == null) aList[i].target='popupFrame';
			}
			var fList  = window.frames["popupFrame"].document.getElementsByTagName('form');
			for (i = 0; i < fList.length; i++) {
				if (fList.target == null) fList[i].target='popupFrame';
			}
		}
	}
}
