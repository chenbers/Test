 ...
 */
function MarkerClustererWithStackedMarkers(map, opt_markers, clusterMarker_opts, opt_opts) {
  // private members
  var clusters_ = [];
  var map_ = map;
  var maxZoom_ = null;
  ...
  var leftMarkers_ = [];
  var mcfn_ = null;
  var clusterMarker_opts_ = clusterMarker_opts;
  var i = 0;
  for (i = 1; i <= 5; ++i) {
    styles_.push({
  ...
  this.addMarker = function (marker, opt_isNodraw, opt_isAdded, opt_clusters, opt_isNoCheck) {
    ...
    cluster = new LabeledCluster(this);
    cluster.addMarker({
      'isAdded': isAdded,
      'marker': marker
    });
   ...
   * @return {opts}
   */
  this.getClusterMarker_Opts_ = function () {
    return clusterMarker_opts_;
  };
 ...
 */
function LabeledCluster(markerClusterer) {
  var center_ = null;
  var markers_ = [];
...
//  var iconImage_ =  markerClusterer.getIconImage_();
  ...
  var opts_ = markerClusterer.getClusterMarker_Opts_();
...
//        clusterMarker_ = new ClusterMarker_(center_, this.getTotalMarkers(), markerClusterer_.getStyles_(), markerClusterer_.getGridSize_());
  ...
  this.clearMarkers = function () {
    if (labeledMarker_ !== null) {
      map_.removeOverlay(labeledMarker_);
    }
    for (var i = 0; i < markers_.length; ++i) {
      if (markers_[i].isAdded) {
        map_.removeOverlay(markers_[i].marker);
