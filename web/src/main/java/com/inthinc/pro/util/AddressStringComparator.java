...
public class AddressStringComparator implements Comparator<String> {

	Locale locale;

	public AddressStringComparator(Locale locale) {
		super();
		this.locale = locale;
	}
	...
	public int compare(String thisString, String thatString) {
		...
		for (i = Math.min(thisTokens.length, thatTokens.length)-1; i>=0 ;i--){
		...
		if (thisTokens.length < thatTokens.length) return 1;
		if (thatTokens.length < thisTokens.length) return -1;
...
//		System.out.println(test31+", "+test32+ " "+test31.compareTo(test32));
		...
		list.add("7325 Prospector Drive Salt Lake City Utah 84121");
		list.add("7329 Prospector Drive Salt Lake City Utah 84121");
		list.add("3325 Prospector Drive Salt Lake City Utah 84121");
		list.add("7325 Prospector Drive Salt Lake City Utah 84104");
		list.add("abc");
		list.add("1abc");
		list.add("abc 11");
		list.add("abc 2");
		...
		Collections.sort(list, new AddressStringComparator(Locale.GERMAN));
		...
		Collections.sort(list, new AddressStringComparator(Locale.US));
