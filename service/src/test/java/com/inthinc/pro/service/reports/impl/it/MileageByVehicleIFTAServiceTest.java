...
public class MileageByVehicleIFTAServiceTest extends BaseEmbeddedServerITCase {
    ...
    public void testGetMileageByVehicleWithDates() {
        ...
        ClientResponse<List<MileageByVehicle>> response = client.getMileageByVehicleWithDates(GROUP_ID_WITH_DATA, TEST_START_DATE, TEST_END_DATE);
    ...
    public void testGetMileageByVehicleWithIftaAndDates() {
        ...
        // test case when Group has data
        ClientResponse<List<MileageByVehicle>> response = client.getMileageByVehicleWithIftaAndDates(GROUP_ID_WITH_DATA, TEST_START_DATE, TEST_END_DATE);
        // assertEquals(Status.OK.getStatusCode(), response.getStatus());
        // List<MileageByVehicle> list = response.getEntity();
        // assertTrue(list.size() > 0);
    }
    ...
    public void testGetMileageByVehicleDefaults() {
        ...
        ClientResponse<List<MileageByVehicle>> response = client.getMileageByVehicleDefaults(GROUP_ID_NOT_IN_HIERARCHY);
        // TODO fix it
...
//    @Test
    public void testValidationOfParameters() {
        ClientResponse<List<MileageByVehicle>> response = client.getMileageByVehicleWithDates(-1, "19790303", "20100303");
        // Verifies the body can be unmarshalled as string for stack trace.
        response.getEntity(String.class);
        assertEquals(Status.BAD_REQUEST.getStatusCode(), response.getStatus());

        client.getMileageByVehicleWithDates(-1, "20100303", "19790303");
        // Verifies the body can be unmarshalled as string for stack trace.
        response.getEntity(String.class);
        assertEquals(Status.BAD_REQUEST.getStatusCode(), response.getStatus());
