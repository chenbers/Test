...
@Service
public class ForwardCommandHttpService {
    @Autowired
    ForwardCommandHttpRepository forwardCommandRepository;
    
	public Number getForwardCommandCount(){
		return forwardCommandRepository.count();
	}

	public ForwardCommandHttp getForwardCommandByID(Integer id){
	...
	public ForwardCommandHttp queueForwardCommand(ForwardCommandHttp forwardCommand){
		/*
		 * http
		 * IF _intData = -1 THEN
			SET _intData  = null;
