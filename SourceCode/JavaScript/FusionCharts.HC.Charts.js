        ...
        chartAPI('pie2d', {
            ...
            eiMethods: {
                sliceDataItem: function(index) {
                    ...
                    if (hcObj && hcObj.datasets &&
                        (dataset = hcObj.datasets[0]) && dataset.data &&
                        dataset.data[index] && dataset.data[index].plot) {

                        return hcObj.plotGraphicClick.call(dataset.data[index].plot);
                    }
                }
            }
        }, singleSeriesAPI);
       ...
       //Pie 3D point class
        function pie3DManager (x, y, r, innerR, radiusYFactor, depth, seriesGroup,
            ...
            this.slicingWallsFrontGroup = renderer.group('slicingWalls-front-Side',
                                                                    seriesGroup),
            //this.outerFrontGroup = renderer.group('outer-front-Side', seriesGroup),
        ...
        pie3DManager.prototype = {
                    ...
                    if (!doNotApply) {
                        ...
                        if (pointConf.elements.frontOuter1) {
                            ...
                            pointConf.elements.frontOuter1.attr({
                                'clip-path': pointConf.clipOuterFrontPath1
                            });
                        }

                        pointConf.thisElement.attr({
                            'clip-path': pointConf.clipTopPath
                        });
                        ...
                        pointConf.elements.topBorder.attr({
                            'clip-path': pointConf.clipTopPath
                        });
                        pointConf.elements.frontOuter.attr({
                            'clip-path': pointConf.clipOuterFrontPath
                        });
                        pointConf.elements.backOuter.attr({
                            'clip-path': pointConf.clipOuterBackPath
                        });
                        if (this.isDoughnut) {
                            pointConf.elements.backInner.attr({
                                'clip-path': pointConf.clipInnerBackPath
                            });
                            pointConf.elements.frontInner.attr({
                                'clip-path': pointConf.clipInnerFrontPath
                            });
                        }
                        if (this.hasOnePoint) {
                    ...
                    if (startOri != endOri) {
                        if(start > end) {//crossed the 0 deg line
                            if (start < pi){//crossed the 180 deg line also
                                ...
                                pointConf.outerFrontPath = moveCmdArr.concat(rightPoint, tempArr3, lineCmdArr, [endOuterX, endOuterY1], getArcPath(cx, depthY, endOuterX, endOuterY1, this.rightX, depthY, rx, ry, 0, 0), closeCmdArr);
                                pointConf.outerBackPath = moveCmdArr.concat( [startOuterX, startOuterY], tempArr1, lineCmdArr, rightDepthPoint, getArcPath(cx, depthY, this.rightX, depthY, startOuterX, startOuterY1, rx, ry, 0, 0), closeCmdArr);
                    ...
                    if (!doNotApply) {
                        ...
                        if (pointConf.elements.frontOuter1) {
                            ...
                            elements.frontOuter1.attr({
                                path: pointConf.outerFrontPath1
                            });
                        }
                        pointConf.thisElement.attr({
                            path: pointConf.topPath
                        });
                        elements.topBorder.attr({
                            path: pointConf.topBorderPath
                        });
                        ...
                        elements.frontOuter.attr({
                            path: pointConf.outerFrontPath
                        });
                        elements.backOuter.attr({
                            path: pointConf.outerBackPath
                        });
                        if (this.isDoughnut) {
                            elements.frontInner.attr({
                                path: pointConf.innerFrontPath
                            });
                            elements.backInner.attr({
                                path: pointConf.innerBackPath
                            });
                        }
            ...
            createSlice: (function () {
                ...
                return function (start, end, color, alpha, borderColor, borderWidth, positionIndex, toolText) {
                    ...
                    /* @note
                     * To bypass the FFv17/18 event loosing issue on clipping,
                     * for SVG, the relevant faces are contained in a group. The
                     * faces are top, outerFront and innerBack only. Borders are
                     * left as they have no effect in this case. Inner side faces
                     * do not have clipping applied on them.
                     */
                    // top
                    if (hasSVG && !hasTouch) {
                        // To bypass the FFv17/18 event loosing issue on clipping
                        returnElement = renderer.group(this.topGroup);
                        renderer.path(confObject.topPath, returnElement)
                        .attr({
                        ...
                        returnElement = renderer.path(confObject.topPath, this.topGroup)
                        .attr({
                            fill: toRaphaelColor(colorObj.top),
                            'stroke-width' : 0
                        });
                    ...
                    // top border
                    elements.topBorder = renderer.path(confObject.topBorderPath, this.topGroup)
                    .attr({
                        stroke: borderColor,
                    ...
                    // bottom
                    elements.bottom = renderer.path(confObject.topPath, this.bottomBorderGroup)
                    .attr({
                    ...
                    // bottom
                    elements.bottomBorder = renderer.path(confObject.topBorderPath, this.bottomBorderGroup)
                    .attr({
                        stroke: borderColor,
                        'stroke-width' : borderWidth
                    });
                    ...
                    if (hasSVG && !hasTouch) {
                        // To bypass the FFv17/18 event loosing issue on clipping
                        elements.frontOuter = renderer.group(this.slicingWallsFrontGroup);
                        renderer.path(confObject.outerFrontPath, elements.frontOuter)
                        .attr({
                        ...
                        elements.frontOuter = renderer.path(confObject.outerFrontPath, this.slicingWallsFrontGroup)
                        .attr({
                            fill: toRaphaelColor(colorObj.frontOuter),
                            'stroke-width' : 0
                        });
                    ...
                    // outerback
                    elements.backOuter = renderer.path(confObject.outerBackPath, this.outerBackGroup)
                    .attr({
                    ...
                    // startSlice
                    //whenAtBack
                    elements.startSlice = renderer.path(confObject.startSlice, this.slicingWallsFrontGroup)
                    .attr({
                    ...
                    // ensSlice
                    //whenAtBack
                    elements.endSlice = renderer.path(confObject.endSlice, this.slicingWallsFrontGroup)
                    .attr({
                    ...
                    if (scaleAngle > pi) {
                        // outerFront
                        elements.frontOuter1 = renderer.path(confObject.outerFrontPath1, this.slicingWallsFrontGroup)
                        .attr({
                            fill: toRaphaelColor(colorObj.frontOuter),
                            'stroke-width' : 0
                        });
                        ...
                        if (hasSVG) {
                            ...
                            elements.frontOuter1.attr({'clip-path': confObject.clipOuterFront1});
                        }
                    }
                    else {
                        //add all slice in slicingWallsArr
                    ...
                    if (this.isDoughnut) {
                        // innerFront
                        elements.frontInner = renderer.path(confObject.innerFrontPath, this.innerFrontGroup)
                        .attr({
                        ...
                        if (hasSVG && !hasTouch) {
                            // To bypass the FFv17/18 event loosing issue on clipping
                            elements.backInner = renderer.group(this.innerBackGroup);
                            renderer.path(confObject.innerBackPath, elements.backInner)
                            .attr({
                            ...
                            elements.backInner = renderer.path(confObject.innerBackPath, this.innerBackGroup)
                            .attr({
                                fill: toRaphaelColor(colorObj.backInner),
                                'stroke-width' : 0
                            });
                    ...
                    if (hasSVG) {
                        ...
                        returnElement.attr({'clip-path': confObject.clipTop});
                        elements.topBorder.attr({'clip-path': confObject.clipTop});
                        elements.bottom.attr({'clip-path': confObject.clipTop});
                        elements.bottomBorder.attr({'clip-path': confObject.clipTop});
                        elements.frontOuter.attr({'clip-path': confObject.clipOuterFront});
                        elements.backOuter.attr({'clip-path': confObject.clipOuterBack});

                        if (this.isDoughnut) {
                            confObject.clipInnerFront = confObject.clipInnerFrontPath;
                            confObject.clipInnerBack = confObject.clipInnerBackPath;

                            elements.frontInner.attr({'clip-path': confObject.clipInnerFront});
                            elements.backInner.attr({'clip-path': confObject.clipInnerBack});
                        }
                    }
        ...
        renderer('renderer.pie3d', {
            ...
            drawPlotPie3d: function(plot, dataOptions) {
                ...
                seriesData.singletonCase = (plotData.length == 1);
                ...
                while (++i < plotData.length) {
                    ...
                    if (!(plotItem = plotItems[i])) {
                        dataOptions.data[i].plot = plotItem = plotItems[i] = {
                            ...
                            graphic: dataset.pie3DManager.createSlice(shapeArgs.start, shapeArgs.end, set.color, set._3dAlpha, set.borderColor, set.borderWidth, i, (isTooltip ? toolText : ''))
                        };

                        // attach legend click event handler for slice
                        dataOptions.data[i].legendClick = (function (i) {
                                return function () {
                                    chart.legendClick(null, chart, i);
                                };
                            }(i));
            ...
            placeDataLabels: (function () {
                ...
                return function(isRotating, plotItems) {
                    var chart = this,
                        ...
                        dataLabelsRadiusY = dataset.labelsRadiusY,
                        excess,
                        excessArr,
            ...
            legendClick: function(item, chart, id) {
                var plots = chart.elements.plots[0];
                plots.isRotating = false;
                chart.plotGraphicClick.call(plots.items[id]);
            }
                ...
                while (i--) {
                    ...
                    if (!(plotItem = plotItems[i])) {
                        dataOptions.data[i].plot = plotItem = plotItems[i] = {
                            ...
                            sliced: sliced,
                            labelText: displayValue,
                            graphic: paper.ringpath(cx, cy, r, r2, startAngle, startAngle, layers.dataset).attr({
...
FusionCharts(['private', 'modules.renderer.js-zoomline', function () {
    var global = this,
        ...
        DOT = '.',
        BLANK = '',
        TRACKER_FILL = 'rgba(192,192,192,'+ (R.svg ? 0.000001 : 0.002) +')', // invisible but clickable
        UNDEF,
    ...
    chartapi('zoomline', {
        ...
        chart: function () {
            ...
            extend2(optionsChart, {
                ...
                btnResetChartTooltext: pluck(chartDef.btnresetcharttooltext, 'Reset Zoom'),
        ...
        preSeriesAddition: function () {
            var definition = this.dataObj,
                ...
                rotateLabels = (chartDef.labeldisplay && chartDef.labeldisplay.toLowerCase() === 'rotate') ? 270 :
            ...
            if (labelHeight > conf.height * 0.5) {
                labelHeight = conf.height * 0.5;
            }

            cats.labelHeight = labelHeight && (labelHeight + 5) || 0;
            ...
            conf.marginBottomExtraSpace += labelHeight;
        ...
        series: function () {
            ...
            if (!inmchar && !inxchar && cdm && forceLimits &&
                    userYMax !== undefined && userYMin !== undefined) {
                forceLimits = true;
            ...
            ((ppp = pluckNumber(chartDef.pixelsperpoint, 15)) < 15) && (ppp = 15);
    ...
    renderer('renderer.zoomline', {
        ...
        pinRange: function (pxs, pxe) {
            ...
            pinclip[2] = pxw;
            pingroup.attr({
                'transform': ['T', chart._visx, chart._yzero],
                'clip-rect': pinclip
            }).show();
        ...
        drawGraph: function () {
            ...
            if (allowpin)  {
                ...
                pinclip = elements['clip-pinrect'] =
                    [tAtt.x, tAtt.y - 1, tAtt.width, tAtt.height];

                pingroup = layers.zoompin = paper.group('zoompin')
                    .insertBefore(datalayer)
                    .transform(['T', visx, yzero]).hide();
                ...
                pinrect = elements.pinrect = paper.rect(0, plotY - yzero,
                    ...
                    ishot: true,
                    cursor: 'w-resize'
                }).drag(function (_dx, _dy, _x, _y, evt) {
                    // transform group
                    // reapply clip after transform
                }, function (x) {
                });
                ...
                chart.pinButton = toolbar.add('pinModeIcon', function () {
                    var info = chart._zoominfo,
                        pinButton = chart.pinButton;

                    if (info.pinned) {
                        chart.pinRange(); // reset pin
                        tooltip && pinButton
                            .tooltip(optionsChart.btnSwitchToPinModeTooltext || BLANK);
                    }
                    else {
                        info.pinned = true;
                        tooltip && pinButton
                            .tooltip(optionsChart.btnSwitchToZoomModeTooltext || BLANK);

                    }
                    pinButton.attr('button-active', info.pinned);
                }, {
            ...
            elements.zoomscroller = paper.scroller(tAtt.x, tAtt.y, tAtt.width, tAtt.height, true, {
                showButtons: true,
                scrollRatio: info.vdl / (info.clen - !!info.clen),
                scrollPosition: [info.dsi / (info.clen - info.vdl - 1), false]
            }, scrollerLayer)
                .attr('fill', optionsChart.scrollColor)
            ...
            bindSelectionEvent(chart, function (o) {
                ...
                chart[chart._zoominfo.pinned ? 'pinRange' : 'zoomRange'](pxs, pxe);
            }, {
                attr: {
            ...
            // Create the crosshair and its related events.
        ...
        drawPlotZoomlineGraphics: function (info, data, element, anchors, labels) {
            ...
            for (; i <= ii; i += step, p += step) {
                ...
                if (label && !(d % lskip)) {
                    attrs = label.attrs;
                    text = labelTexts[i] || BLANK;
                    lx = (lx < 0 || lx > visw) ? outside : px; // calculate bounds

                    attrs.text === text ?
                        (delete attrLabel.text) : (attrLabel.text = text);
                    attrs[lcmd] === lx ?
    ...
    CrossLine = function (chart)  {
        ...
        var paper = chart.paper,
            ...
            line = this.line = paper.path(undefined, datalayer).attr({
    ...
    CrossLine.prototype.updateLabels = function () {
        ...
        labels.forEach(function (label, i) {
            value = plots[i].data[position];
            label.attr({
                text: value + BLANK,
                x:  mathFloor(x),
                y: value === UNDEF ? -1e3 : value * pvr,
