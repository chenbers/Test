...
public class ParseStepForElementMethodTest {
	...
	public void testDefaultCreds() {
		...
		assertEquals("Unable to get the default user from the string", apb
				.getMainAutomation().get(1), getUsername());
		...
		assertEquals("Unable to get the default user from the string", apb
				.getEditableAccount().get(1), getUsername());
		assertEquals("Unable to get the default user from the string",
				apb.getPassword(), getPassword());
	...
	private String getPassword() {
		return AutoStepVariables
				.getComparator("When I type my password into the Password field");
	}

	private String getUsername() {
		return AutoStepVariables
				.getComparator("When I type my user name into the Username field");
	}
	...
	public void testQuotedValue() {
		...
		String stepAsString = "I type \"" + value
				+ "\" into the Username field";
		assertEquals("Was unable to get the value from the step", value,
				AutoStepVariables.getComparator(stepAsString));
	}

	private final static String[] variableStrings = {
			"This is my test value 321 !@#$", "Idle &#37;",
	...
	public void testSavingVariable() {
		for (String value : variableStrings) {
			...
			for (String variableName : variableNames) {
				String saveStep = "Then I save the Login button as "
						+ variableName;
				AutoStepVariables.setComparator(saveStep, value);
				String useStep = "I type " + variableName
						+ " into the Login field";
				assertEquals("Was unable to get the value from the step",
						unescaped, AutoStepVariables.getComparator(useStep));
			}
		}
	}
		...
		for (String value : variableStrings) {
			...
			for (String variableName : variableNames) {
				String saveStep = "Then I save the Login button as "
						+ variableName;
				AutoStepVariables.setComparator(saveStep, value);
				String useStep = "I validate the Login button is "
						+ variableName;
				assertEquals("Was unable to get the value from the step",
						unescaped, AutoStepVariables.getComparator(useStep));
			}
	...
	public void testCompositeDefaultLoginStep() {
		...
		candidate.addComposedSteps(composedSteps, mainAutomationStep,
		...
		assertThat("The Usernames didn't match",
				test.getMyUser().getUsername(),
				is(apb.getMainAutomation().get(1)));
		assertThat("The Usernames didn't match", getUsername(), is(apb
				.getMainAutomation().get(1)));
		assertEquals(
				"The password didn't match, if it is changed it should be fixed",
				true, test.getMyUser().doesPasswordMatch(apb.getPassword()));
		assertThat(
				"The password didn't match, if it is changed it should be fixed",
				getPassword(), is(apb.getPassword()));
	}
	...
	public void testCompositeEditAccountLoginStep() {
		String editAccountStep = "Given I am logged in "
				+ AutoCustomSteps.getEditableaccountuser();
		CustomAutomationSteps steps = new CustomAutomationSteps();
		List<StepCandidate> candidates = steps.listCandidates();
		StepCandidate candidate = candidateMatchingStep(candidates,
				editAccountStep);
		...
		List<Step> composedSteps = new ArrayList<Step>();
		candidate.addComposedSteps(composedSteps, editAccountStep,
		...
		assertThat("The Usernames didn't match",
				test.getMyUser().getUsername(), is(apb.getEditableAccount()
						.get(1)));
		assertThat("The Usernames didn't match", getUsername(), is(apb
				.getEditableAccount().get(1)));
		assertEquals(
				"The password didn't match, if it is changed it should be fixed",
				true, test.getMyUser().doesPasswordMatch(apb.getPassword()));
		assertThat(
	...
	public void testCompositeEditUserLoginStep() {
		String editAccountStep = "Given I am logged in "
				+ AutoCustomSteps.getEditableuser();
		CustomAutomationSteps steps = new CustomAutomationSteps();
		List<StepCandidate> candidates = steps.listCandidates();
		StepCandidate candidate = candidateMatchingStep(candidates,
				editAccountStep);
	...
	public void testParameterParser() {
		...
		for (int i = 0; i < numOfTimesToLoop; i++) {
			...
			String falseStep = "Then I am not getting " + randInt
					+ " field as my \"" + string + "\"";

			for (Method method : parseMethods) {
				try {
					Object[] params = AutoMethodParser.getParameters(falseStep,
							method);
		...
		for (int i = 0; i < numOfTimesToLoop; i++) {
			randInt = rand.getInt(Integer.MAX_VALUE);
			string = rand.getCharString(15);
			String trueStep = "Then I will get " + randInt + " field as my \""
					+ string + "\"";

			for (Method method : parseMethods) {
				try {
					Object[] params = AutoMethodParser.getParameters(trueStep,
							method);
	...
	private void checkParams(Object[] params, Boolean bool, Integer integer,
			String string) {
		for (Object param : params) {
			if (param instanceof Integer) {
				assertEquals(
						"Parameter Parser didn't parse the Integer correctly",
						integer, param);
			} else if (param instanceof Boolean) {
				assertEquals(
						"Parameter Parser didn't parse the Boolean correctly",
						bool, param);
			} else {
				assertEquals(
						"Parameter Parser didn't parse the String correctly",
						string, param);
			}
		}
	}
	...
	static {
		...
		try {
			...
			parseMethods.add(clazz.getMethod("getOneOfEach", Boolean.class,
					Integer.class, Object.class));
			parseMethods.add(clazz.getMethod("getOneOfEach", boolean.class,
					int.class, String.class));
		} catch (SecurityException e) {
			Log.error(e);
			throw new NullPointerException("Could not add all of the methods: "
	...
	@Test
	public void testParsingCalendar() throws SecurityException,
			NoSuchMethodException {
		RandomValues rand = new RandomValues();
		SeleniumEnumWrapper id = new SeleniumEnumWrapper("TestingParsing",
				"someFakeID");
		CalendarObject co = new CalendarObject(id);
		Method method = co.getClass().getMethod("select",
				AutomationCalendar.class);
		for (int i = 0; i < numOfTimesToLoop; i++) {
			int epochStamp = Integer.parseInt(rand.getIntString(9)) + 1000000000;
			for (WebDateFormat format : EnumSet.allOf(WebDateFormat.class)) {
				AutomationCalendar time = new AutomationCalendar(epochStamp,
						format);
				String step = "And I select \"" + time
						+ "\" in the Start dropdown";
				Object[] params = AutoMethodParser.getParameters(step, method);
				assertEquals("We should only have gotten 1 parameter", 1,
						params.length);
				assertEquals("The date was not parsed correctly", time,
						params[0]);
			}
		}
	}
	...
	@Test
	public void testParsingCalendarFromVariable() throws SecurityException,
			NoSuchMethodException {
		RandomValues rand = new RandomValues();
		String variableName = "my big long variable that you will not guess";
		String saveStep = "I want to save this param for Start field as "
				+ variableName;
	...
	private static final String[][] textValidationSteps = {
			...
			{ "the Confirm Password Error text",
					"New and Confirm New Password do not match" }
	 ...
	 * The array of arrays represent each way a method can be found in a step
	 * string. <br />
	 * The first entry in each array is "is" or nothing, and the second entry is
	 * what we <br />
	...
	static {
		testTextMethods.put("assertTheSameAs", new String[][] {
				{ "is", "the same as" }, { "is", "" } });
		testTextMethods.put("assertIsNot", new String[][] { { "is", "is not" },
				{ "is", "not the same as" } });
		testTextMethods.put("assertContains", new String[][] {
				{ "", "contains" }, { "is", "close to" } });
		testTextMethods.put("assertDoesNotContain", new String[][] {
				{ "", "does not contain" }, { "is", "not close to" } });

		testTextMethods.put("validate", new String[][] { { "is", "" },
				{ "is", "the same as" } });
		testTextMethods.put("validateIsNot", new String[][] { { "is", "not" },
				{ "is", "not equal to" } });
		testTextMethods.put("validateContains", new String[][] {
				{ "", "contains" }, { "is", "close to" } });
		testTextMethods.put("validateDoesNotContain", new String[][] {
				{ "", "does not contain" }, { "is", "not close to" } });
	}

	@Test
	public void testValidationParser() {
		for (Map.Entry<String, String[][]> methodEntry : testTextMethods
				.entrySet()) {
			String methodName = methodEntry.getKey();
			String testPart = methodName.contains("validate") ? "validate"
					: "assert";
			for (String[] possibles : methodEntry.getValue()) {
				...
				for (String[] validation : textValidationSteps) {
					...
					String stepAsString = String.format(
							"And I %s %s %s %s \"%s\"", testPart, stepPart,
							isPart, methodPart, stringPart);
					try {
						PendingStep step = new PendingStep(stepAsString,
								stepAsString);
						Map<Method, Object[]> methods = AutoMethodParser
								.parseValidationStep(step, Text.class);

						for (Map.Entry<Method, Object[]> entry : methods
								.entrySet()) {
							assertEquals("Wrong method for step: "
									+ stepAsString, methodName, entry.getKey()
									.getName());
							assertEquals("Parser retrieved incorrect variable",
									stringPart, entry.getValue()[0]);
						}
					} catch (NoSuchMethodException e) {
						fail("Was unable to find method: " + methodName
								+ " for step \"" + stepAsString + "\"");
					}
				}
			}
		}
	...
	public static final String[] varNames = { "TRAILER", "LOCATION",
			"this is my variable" };
			...
			for (Map.Entry<String, String[][]> methodEntry : testTextMethods
				...
				for (String[] possibles : methodEntry.getValue()) {
					...
					for (String[] validation : textValidationSteps) {
						...
						String stepAsString = String.format(
								"And I %s %s %s %s %s", testPart, stepPart,
								isPart, methodPart, variableName);
						try {
							PendingStep step = new PendingStep(stepAsString,
									stepAsString);
							Map<Method, Object[]> methods = AutoMethodParser
							...
							for (Map.Entry<Method, Object[]> entry : methods
									.entrySet()) {
								assertEquals("Wrong method for step: "
										+ stepAsString, methodName, entry
										.getKey().getName());
								assertEquals(
										"Parser retrieved incorrect variable for step: "
												+ stepAsString, stringPart,
										entry.getValue()[0]);
							}
						} catch (NoSuchMethodException e) {
							fail("Was unable to find method: " + methodName
	...
	private static final String[] booleanSteps = { "the Sort By Number link",
	...
	static {
		testBooleanMethods.put("assertPresence", new String[][] { { "is",
				"present" } });
		testBooleanMethods.put("assertVisibility", new String[][] { { "is",
				"visible" } });
		testBooleanMethods.put("assertChecked", new String[][] { { "is",
				"checked" } });
		testBooleanMethods.put("assertClickable", new String[][] { { "is",
				"clickable" } });

		testBooleanMethods.put("validatePresence", new String[][] { { "is",
				"present" } });
		testBooleanMethods.put("validateVisibility", new String[][] { { "is",
				"visible" } });
		testBooleanMethods.put("validateChecked", new String[][] { { "is",
				"checked" } });
		testBooleanMethods.put("validateClickable", new String[][] { { "is",
				"clickable" } });
	}

	@Test
	public void testBooleanValidationParser() {
		for (Map.Entry<String, String[][]> methodEntry : testBooleanMethods
				.entrySet()) {
			String methodName = methodEntry.getKey();
			String testPart = methodName.contains("validate") ? "validate"
					: "assert";
			for (String[] possibles : methodEntry.getValue()) {
				...
				for (String stepPart : booleanSteps) {
					...
					for (int i = 0; i < 4; i++) {
						...
						String stepAsString = String.format(
								"And I %s %s %s %s %s", testPart, stepPart,
								isPart, booleanPart, methodPart);
						try {
							PendingStep step = new PendingStep(stepAsString,
									stepAsString);
							Map<Method, Object[]> methods = AutoMethodParser
									.parseValidationStep(step, CheckBox.class);
							...
							for (Map.Entry<Method, Object[]> entry : methods
								...
								assertEquals("Wrong method for step: "
										...
										.getKey().getName());
								assertEquals(
										"Parser retrieved incorrect variable",
	...
	private static final String[] textTableSteps = { "the Sort By Number link",
			"the Status Filter dropdown" };
	...
	static {
		textTableBooleanMethods.put("assertPresence", new String[][] { { "is",
				"present" } });
	}

	@Test
	public void testTextTableBooleanValidationParser() {
		for (Map.Entry<String, String[][]> methodEntry : textTableBooleanMethods
				.entrySet()) {
			String methodName = methodEntry.getKey();
			String testPart = methodName.contains("validate") ? "validate"
					: "assert";
	...
	private static final String[] textLinkTableSteps = {
			"the Sort By Number link", "the Status Filter dropdown" };
	...
	static {
		...
		textLinkTableMethods.put("assertPresence", new String[][] { { "is",
				"present" } });
	}

	// @Test
	// public void testBooleanValidationParser(){
	// for (Map.Entry<String, String[][]> methodEntry :
	// textLinkTableMethods.entrySet()){
	// String methodName = methodEntry.getKey();
	...
	// AutoMethodParser.parseValidationStep(step, CheckBox.class);
	//
	...
	// }
	// } catch (NoSuchMethodException e){
	// fail("Was unable to find method: " + methodName + " for step \"" +
	// stepAsString + "\"");
	// }
	// }
	// }
	...
	// * The array of arrays represent each way a method can be found in a step
	// string. <br />
	// * The first entry in each array is "is" or nothing, and the second entry
	// is what we <br />
	// * are expecting to use to match the method to the step.
	// */
	// private static final Map<String, String[][]> testBooleanMethods = new
	// HashMap<String, String[][]>();
	// static {
	// testBooleanMethods.put("assertPresence", new
	// String[][]{{"is","present"}});
	// testBooleanMethods.put("assertVisibility", new
	// String[][]{{"is","visible"}});
	// testBooleanMethods.put("assertChecked", new
	// String[][]{{"is","checked"}});
	// testBooleanMethods.put("assertClickable", new
	// String[][]{{"is","clickable"}});
	//
	// testBooleanMethods.put("validatePresence", new
	// String[][]{{"is","present"}});
	// testBooleanMethods.put("validateVisibility", new
	// String[][]{{"is","visible"}});
	// testBooleanMethods.put("validateChecked", new
	// String[][]{{"is","checked"}});
	// testBooleanMethods.put("validateClickable", new
	// String[][]{{"is","clickable"}});
	// }
	//
	//
