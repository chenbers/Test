...
public class PrettyJSON {
	...
	public static String toString(Object item) {
		...
		try {
			if (item instanceof JSONObject) printJSONObject(aStringAString, (JSONObject) item);
			else if (item instanceof JSONArray) printJSONArray(aStringAString, (JSONArray) item);
	 ...
	 private static void printJSONObject(StringWriter aStringAString, JSONObject item) throws JSONException {
    	...
    	while (itr.hasNext()) {
    		...
    		if (item.get(next) instanceof String || item.get(next)==null) {
    			...
    			aStringAString.write(StringUtils.repeat("\t",depth)+"]\n");
    		}else printLine(aStringAString, next);
	...
	private static void printJSONArray(StringWriter stringMeUp, JSONArray array) throws JSONException {
		...
		for (int i=0;i<array.length();i++) {
			...
			if (array.get(i) instanceof String) printLine(stringMeUp, array.getString(i));
			...
			if (i<array.length()-1) stringMeUp.write(StringUtils.repeat("\t",depth));
		}
		depth -=1;
    }
}
