...
public abstract class Base {
    ...
    private void ackFwdCmds( List<HashMap<String, Object>> reply){
    	...
    	if (!reply.isEmpty()){
    		...
    		while (itr.hasNext()){
    			fwd = itr.next();
    			if (fwd.get("fwdID").equals(100) ) continue;
    			createAckNote(fwd);
    		}
    	}
    }
...
//		}
	...
	protected void set_MSP( Integer version ){
        MSP = version;
    }
	
	protected void set_MSP( Object version ){
        MSP = (Integer) version;
    }
...
//    	ProcessServer url = new ProcessServer(server);
//    	set_url(url.getUrl()[2], url.getUrl()[3]);
//    }
//    
