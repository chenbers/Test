...
public abstract class AbstractPage extends MasterTest implements Page {
    ...
    public TextLink getLinkByText(String text) {
        return new TextLink(new SeleniumEnumWrapper("GET_LINK_BY_TEXT", "link=" + text));
    }
    ...
    public AbstractPage assertURL() {
        ...
        if (!results)
            addError("validateURL", getCurrentLocation() + " does not contain " + getExpectedPath() + " ?", ErrorLevel.FATAL);
        return this;
    }
    ...
    public AbstractPage validate() {
        addError("AbstractPage.validate()", "automation cannot validate AbstractPage OR there is no validate() method for the concrete page being tested.", ErrorLevel.FATAL);
        return this;
    }
}
