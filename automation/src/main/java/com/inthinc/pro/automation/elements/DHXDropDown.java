...
public class DHXDropDown extends DropDown implements Selectable {
	 ...
	 * 
	 * @param anEnum
	 *            NOTE: SeleniumEnums in <code>anEnum</code> must be in the
	 ...
	 * @param anEnum
	 * @param enums
	 *            NOTE: SeleniumEnums in <code>enums</code> must be in the order
	 ...
	 * @param replaceWord
	 * @param enums
	 *            NOTE: the SeleniumEnums in <code>enums</code> must be in the
	 *            order they are added to the page.
	 */
	public DHXDropDown(SeleniumEnums anEnum, String replaceWord,
			SeleniumEnums[] enums) {
	...
	/**
	 * Consolidates initialization of new DHXDropDown Objects into a single
	 * method.
	 * 
	 * @param anEnum
	 * @param replaceWord
	 * @param enums
	 *            NOTE: enums in <code>enums</code> must be in the order they
	 *            are added to the page.
	 */
	private void init(SeleniumEnums anEnum, String replaceWord,
			SeleniumEnums[] enums) {
		page = replaceWord;
		if (page != null) {
	...
	/**
	 * Assigns slow/complicated xpaths for DHXDropDown elements on a page to
	 * ID's for easy retrieval. NOTE: if <code>this</code> Object's enums are
	 * NOT in the same order that the DHXDropDown's appear on the rendered page
	 * they will not be assigned correctly.
	 * 
	...
	private DHXDropDown assignIDs() {
		...
		for (SeleniumEnums enume : enums) {
			if (getSelenium().isElementPresent(
					new SeleniumEnumWrapper(enume).replaceWord(page))
					&& enume != null) {
				Log.debug("//body/div[" + div + "]" + ", " + enume.toString());
				getSelenium().assignId("//body/div[" + div + "]",
						enume.toString());
				div++;
			}
		}
	...
	private void setDropDownID() {
		makeDropDown.setID("//input[@name='"
				+ makeDropDown.replaceWord(page).getIDs()[0] + "']/../img");
	}
	...
	public DHXDropDown selectRow(Integer optionNumber) {
		...
		AutomationThread.pause(3,
				"Pause so the browser has a chance to catch up");
		return this;
	}
	...
	@Override
	public DHXDropDown selectTheOptionContaining(String partialMatch,
			Integer matchNumber) {
		assignIDs();
		matchNumber--;
	...
	private String makeXpath(String secondDiv) {
		String xpath = "//div[" + Id.id(myEnum.toString()) + "]/div["
				+ secondDiv + "]";
		return xpath;
	}
    ...
    public String getText(){
    	...
    	return getSelenium().getText(xpath);
    }
