...
public abstract class AggregationCassandraDAO extends GenericCassandraDAO {
    ...
    protected List<TrendItem> getTrendForAsset(Integer id, EntityType entityType, Integer binSize, Integer count) {
        ...
        List<Composite> rowKeys = createDateIDKeys(getToday(id, entityType), id, binSize, count); // TO DO: set correct timezone
    ...
    protected Map<String, Map<String, Long>> summarizeByID(CounterRows<Composite, String> rows) {
        return summarize(rows, false, true, false, null, null);
    }

    protected Map<String, Map<String, Long>> summarizeByIDHierarchy(CounterRows<Composite, String> rows, GroupHierarchy gh, Integer topGroupID) {
        return summarize(rows, false, true, false, gh, topGroupID);
    }

    protected Map<String, Map<String, Long>> summarizeByDate(CounterRows<Composite, String> rows) {
        return summarize(rows, true, false, false, null, null);
    }

    protected Map<String, Map<String, Long>> summarizeByDateHierarchy(CounterRows<Composite, String> rows, GroupHierarchy gh, Integer topGroupID) {
        return summarize(rows, true, false, false, gh, topGroupID);
    }

    protected Map<String, Map<String, Long>> summarizeByDateID(CounterRows<Composite, String> rows) {
        return summarize(rows, true, true, false, null, null);
    }


    protected Map<String, Map<String, Long>> summarizeByWeekID(CounterRows<Composite, String> rows) {
        return summarize(rows, true, true, true, null, null);
    }


    protected Map<String, Map<String, Long>> summarizeByWeek(CounterRows<Composite, String> rows) {
        return summarize(rows, true, false, true, null, null);
    }

    protected Map<String, Map<String, Long>> summarizeByWeekHierarchy(CounterRows<Composite, String> rows, GroupHierarchy gh, Integer topGroupID) {
        return summarize(rows, true, false, true, gh, topGroupID);
    }

    protected Map<String, Map<String, Long>> summarize(CounterRows<Composite, String> rows) {
        return summarize(rows, false, false, false, null, null);
    }

    protected Map<String, Map<String, Long>> summarizeHierarchy(CounterRows<Composite, String> rows, GroupHierarchy gh, Integer topGroupID) {
        return summarize(rows, false, false, false, gh, topGroupID);
    }

    protected Map<String, Map<String, Long>> summarize(CounterRows<Composite, String> rows, boolean includeAggDate, boolean includeID, boolean bucketByWeek, GroupHierarchy gh, Integer topGroupID) {
        // An Aggregate composite rowkey consists of 1.) AggDate (2.) ID of either driver,vehicle,group
        logger.debug("begin summarize rows.getCount(): " + rows.getCount());
        ...
        for (CounterRow<Composite, String> row : rows) {
            ...
            if (gh == null || gh.isDescendent(topGroupID, Integer.parseInt(id))) {
        ...
        /*
         * //List currently ordered by date descending. Truncate to count. while (keys2ColumnsMap.size() > count) { String key = (String)keys2ColumnsMap.lastKey();
        ...
        logger.debug("end summarize keys2ColumnsMap.size(): " + keys2ColumnsMap.size());
    ...
    protected Integer getResultForScoreType(ScoreType scoreType, Map<String, Long> columnMap) {
        ...
        switch (scoreType) {
            case SCORE_OVERALL:
                speedPenalty = getValue(columnMap, SPEEDPENALTY1) + getValue(columnMap, SPEEDPENALTY2) + getValue(columnMap, SPEEDPENALTY3) + getValue(columnMap, SPEEDPENALTY4)
                        + getValue(columnMap, SPEEDPENALTY5);
                stylePenalty = getValue(columnMap, AGGRESSIVEBRAKEPENALTY) + getValue(columnMap, AGGRESSIVEACCELPENALTY) + getValue(columnMap, AGGRESSIVELEFTPENALTY)
                        + getValue(columnMap, AGGRESSIVERIGHTPENALTY) + getValue(columnMap, AGGRESSIVEBUMPPENALTY);
    ...
    protected String getAggCF(Integer binSize, EntityType entityType) {
        return (entityType.equals(EntityType.ENTITY_DRIVER)) ? getDriverAggCF(binSize) : getVehicleAggCF(binSize);
    }
    ...
    protected String getTodayForGroup(Integer groupID) {
    ...
    protected String getToday(Integer id, EntityType entityType) {
        return (entityType.equals(EntityType.ENTITY_DRIVER)) ? getTodayForDriver(id) : getTodayForVehicle(id);
    }

    protected String getTodayForDriver(Integer driverID) {
        TimeZone tz = TimeZone.getDefault();
        SimpleDateFormat dateFormat = new SimpleDateFormat(DATE_FORMAT);
        dateFormat.setTimeZone(tz);
        return dateFormat.format(new Date());
    ...
    protected String getTodayForVehicle(Integer vehicleID) {
        TimeZone tz = TimeZone.getDefault();
        SimpleDateFormat dateFormat = new SimpleDateFormat(DATE_FORMAT);
        dateFormat.setTimeZone(tz);
        return dateFormat.format(new Date());
    ...
    protected void updateAggregates(Event event, int multiplier) {
        ...
        if (event.getEventType() == EventType.HARD_BRAKE || event.getEventType() == EventType.HARD_ACCEL || event.getEventType() == EventType.HARD_TURN || event.getEventType() == EventType.HARD_VERT)
    ...
    private void updateSpeedingFields(SpeedingEvent event, Map<String, Long> counterColumns, int multiplier) {
        logger.info("updateSpeedingFields distance: " + event.getDistance() + " speedLimit: " + event.getSpeedLimit() + " topSpeed: " + event.getTopSpeed() + " range: "
                + getRange(event.getSpeedLimit()));
