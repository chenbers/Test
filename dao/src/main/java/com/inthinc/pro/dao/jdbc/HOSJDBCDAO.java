...
//            "where p.empid=:employeeId and p.last=:last";
    ...
    private final static String PREVIOUS_LOG_STARTTIME_SQL_GROUPS = "SELECT min(h.logTime) FROM hoslog h, driver d WHERE h.driverID = d.driverID and d.groupID in (:groupIDs)  AND h.logTime < :startTime AND h.status IN (0,1,2,3,4,7,8,24,29,30,32) ORDER by h.logtime desc LIMIT 1";
