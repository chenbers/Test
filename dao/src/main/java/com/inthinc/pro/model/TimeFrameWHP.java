...
public enum TimeFrameWHP implements BaseEnum {
...
//            return new Interval(new DateMidnight(getCurrent().minusWeeks(1), dateTimeZone), new DateMidnight(getCurrent().plusDays(1), dateTimeZone));
    ...
    private static final Logger logger = Logger.getLogger(TimeFrameWHP.class);
    private AggregationDuration driveQDuration;
    private Integer code;
    private static DateTime current;

    TimeFrameWHP(AggregationDuration driveQDuration, Integer code) {
    ...
    private static final Map<Integer, TimeFrameWHP> codeLookup = new HashMap<Integer, TimeFrameWHP>();
    static
    {
        for(TimeFrameWHP d : EnumSet.allOf(TimeFrameWHP.class))
            codeLookup.put(d.getCode(), d);
    }
    
    public static TimeFrameWHP valueOf(Integer code)
    {
